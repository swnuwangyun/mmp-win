...
void Shader::Init(const char*vs, const char*fs) {
	int nFileSize = 0;
	const char*vsCode = (char*)utils::LoadFileContent(vs, nFileSize);
	const char*fsCode = (char*)utils::LoadFileContent(fs, nFileSize);
	GLuint vsShader = utils::CompileShader(GL_VERTEX_SHADER, vsCode);
	if (vsShader == 0) {
		return;
	}
	GLuint fsShader = utils::CompileShader(GL_FRAGMENT_SHADER, fsCode);
	if (fsShader == 0) {
		return;
	}
	mProgram = utils::CreateProgram(vsShader, fsShader);
